...
use polars_axum_models::{Pagination, WorkspaceModel, WorkspaceQueryArgs, WorkspaceStateModel};
...
pub async fn get_workspace_by_name(
    ...
    let mut workspaces = if let Some(organization_name) = organization_name {
        let Some(organization) = get_organization_by_name(organization_name.clone()).await? else {
            return Err(
                anyhow!("No organization with the name {organization_name} was found").into(),
            );
        };
        get_all_workspaces(Some(workspace_name.clone()), Some(organization.id)).await?
    } else {
        get_all_workspaces(Some(workspace_name.clone()), None).await?
    };
...
pub async fn wait_until_active(
    ...
    for _ in 0..max_polls {
        ...
        workspace = CTRL_PLN_CLIENT_GLOBAL
            .call_async(|client| client.get_workspace(workspace.id))
            .await?;
...
pub async fn print_workspaces() -> ApiResult<()> {
    let mut table = Table::new();
    table
        .load_preset(NOTHING)
        .set_header(vec!["NAME", "ID", "STATUS"]);

    for ws in get_all_workspaces(None, None).await? {
...
pub async fn delete_workspace(
    ...
    CTRL_PLN_CLIENT_GLOBAL
        .call_async(|client: &ApiClient| client.delete_workspace(workspace.id))
        .await?;
