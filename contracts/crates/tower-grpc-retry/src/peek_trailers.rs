...
pub struct PeekTrailersBody<B: Body = tonic::body::Body>{
    #[pin]
    inner: Inner<B>
}
}
...
pin_project! {
    #[project = Projection]
    enum Inner<B: Body = tonic::body::Body> {
        /// An empty body.
        Empty,
...
impl<B: Body> PeekTrailersBody<B> {
        ...
        B::Error: Send,
    {
        ...
        if let Version::HTTP_09 | Version::HTTP_10 | Version::HTTP_11 = rsp.version() {
            return Either::Left(future::ready(rsp.map(|inner| Self {
                inner: Inner::Passthru { inner },
            })));
        }
        ...
        if rsp.is_end_stream() {
            ...
            return Either::Left(future::ready(rsp.map(|_| Self {
                inner: Inner::Empty,
            })));
        }
    ...
    fn split_frame(frame: http_body::Frame<B::Data>) -> Option<future::Either<B::Data, HeaderMap>> {
    ...
    B::Error: Unpin,
{
    ...
    fn poll_frame(
        ...
        let this = self.project().inner.project();
        match this {
