...
struct Test {
    ...
    tx: Tx,
    /// The "initial" body.
    initial: ReplayBody<tonic::body::Body>,
    /// Replays the initial body.
    replay: ReplayBody<tonic::body::Body>,
}
...
struct Tx(http_body_util::channel::Sender<Bytes, Status>);
...
fn empty_body_is_always_eos() {
    ...
    let initial = ReplayBody::try_new(tonic::body::Body::empty(), 64 * 1024)
...
async fn caps_buffer() {
    ...
    let (mut tx, body) = http_body_util::channel::Channel::<Bytes, Status>::new(512);
    let mut initial =
    ...
    assert!(err.source().unwrap().is::<Capped>());
...
fn body_too_big() {
    let max_size = 8;
    let mk_body = |sz: usize| -> tonic::body::Body {
        let s = (0..sz).map(|_| "x").collect::<String>();
        tonic::body::Body::new(s)
    };

    assert!(
        ReplayBody::try_new(tonic::body::Body::empty(), max_size).is_ok(),
    ...
    let (mut _tx, body) = http_body_util::channel::Channel::<Bytes, Status>::new(512);
    assert!(
...
fn size_hint_is_correct_for_empty_body() {
    let initial =
        ReplayBody::try_new(Body::empty(), 64 * 1024).expect("empty body can't be too large");
...
async fn size_hint_is_correct_across_replays() {
    ...
    let mut initial = ReplayBody::try_new(Body::new(BODY.to_string()), CAPACITY)
        .expect("empty body can't be too large");
    let mut replay = initial.clone();
...
impl Test {
    fn new() -> Self {
        let (tx, rx) = http_body_util::channel::Channel::<Bytes, Status>::new(512);
        let initial = ReplayBody::try_new(Body::new(rx), 64 * 1024).expect("body too large");
        let replay = initial.clone();
        Self {
            tx: Tx(tx),
            initial,
...
impl http_body::Body for TestBody {
    type Data = <String as http_body::Body>::Data;
